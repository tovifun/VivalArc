## 🗓️ 2026.03.26 | v1.3.0
这次更新主要针对最新的 Vivaldi 7.9，也算是在停更一年多之后，对 VivalArc 做的一次重新整理。

1. 适配了最新版本的 Vivaldi 7.9。
2. 再次简化了 CSS，减少了很多自定义样式，这样在未来 Vivaldi 更新时会更稳定，也更不容易失效。
3. 距离上一次更新已经过去一年多，过去积累下来的一些界面和样式问题，也在这个版本中做了一轮优化。

现在主流浏览器基本都已经陆续加入了原生垂直标签栏，包括 Chrome 在内也是如此。对我来说，Vivaldi 现在最有价值的地方，更多还是它足够高的可定制性。所以未来我还是会继续维护 VivalArc，不过更新重点会更偏向于修复界面 Bug 和兼容性问题，而不是继续扩展太多新的样式分支。
...
4. 也是本次更新最重要的变化(同时也是最多用户反馈的)，自动隐藏的 Tabbar。此方法还是来自 @Zettry [#21](https://github.com/tovifun/VivalArc/issues/21) [#29](https://github.com/tovifun/VivalArc/issues/29)
   1. 当时这个功能还是实验性设计，所以我单独保留成了一个变体目录。
...
有些用户询问如何使用带有标题栏的样式 #26。当时我为不同偏好做过几个变体版本。相关历史说明请参阅：[VivalArc 历史变体说明](../guides/variants-guide-cn.md)。
